...
class StatefulGroupie(Component):
    def __init__(self):
        super().__init__()
        self.mac = str(hex(getnode()))[-4:]
        self.logger.info(f"riaps-{self.mac} active")
        self.currentGroup = None
        self.instName = "n_"+self.mac

    def handleActivate(self):
            self.currentGroup = self.joinGroup("TheGroup","testGroupName")
            self.logger.info("joined group[%s]: %s" % (self.currentGroup.getGroupName(),str(self.currentGroup.getGroupId())))
