...
class Pmu(object):
    ...
    def pmu_handler(self, connection, address, buffer):
        ...
        try:
            while True:
                ...
                if readable:
                    ...
                    while len(received_data) < 4:
                        received_data += connection.recv(4 - len(received_data))
                ...
                if command:
                    if command == 'start':
                        # empty pending/stale data frames
                        ...
                        self.logger.info("[%d] - Replying Header request -> (%s:%d)",
                                         self.pmu_id, address[0], address[1])
                        frame = self.header.convert2bytes()
                        connection.sendall(frame)
                        self.logger.debug("sent: " + frame_to_ascii(frame))

                    elif command == 'cfg1':
                        self.logger.info("[%d] - Replying Configuration frame 1 request -> (%s:%d)", self.pmu_id,
                                         address[0], address[1])
                        frame = self.cfg1.convert2bytes()
                        connection.sendall(frame)
                        self.logger.debug("sent: " + frame_to_ascii(frame))

                    elif command == 'cfg2':
                        self.logger.info("[%d] - Replying Configuration frame 2 request -> (%s:%d)", self.pmu_id,
                                         address[0], address[1])
                        frame = self.cfg2.convert2bytes()
                        connection.sendall(frame)
                        self.logger.debug("sent: " + frame_to_ascii(frame))

                    elif command == 'cfg3':
                        self.logger.info("[%d] - Replying Configuration frame 3 request -> (%s:%d)", self.pmu_id,
                                         address[0], address[1])
                        frame = self.cfg3.convert2bytes()
                        connection.sendall(frame)
                        self.logger.debug("sent: " + frame_to_ascii(frame))

                if sending_measurements_enabled and not buffer.empty():
                    sleep(delay)
                    self.logger.info("[%d] - Sending data frame sent at [%f] -> (%s:%d)", self.pmu_id, time(), address[0],
                                      address[1])
                    frame = buffer.get()
                    connection.sendall(frame)
                    self.logger.debug("sent: " + frame_to_ascii(frame))
            ...
            print('Incoming connection closed by PDC (%s:%d)...' % (address[0], address[1]))
    ...
    def acceptor(self):
        ...
        while True:
            ...
            print('Incoming connection from PDC (%s:%d)...' % (address[0], address[1]))
            conn.close()
