...
def validate_pub_send():
  pub_log_file = os.path.join(perf.LOGS_DIRECTORY, "modelDeployer-test_1_N.log")
  pub_logs = testutilities.get_log_for_test("test_pub_send", pub_log_file, "12:00:00")
  assert "Sent messages: 1" in pub_logs, "Publisher was unable to send messages"
  assert "Sent messages: 5" in pub_logs, "Publisher couldn't send at least 5 messages"
  assert "Received messages: 5" in pub_logs > 2, "Not all the subscribers got the messages"