...
class Actor(object):
    ...
    '''          
    def __init__(self, gModel, aName, sysArgv):
        '''
        Constructor
        '''
        ...
        for instName in instSpecs:                          # Create the component instances: the 'parts'
            ...
            instFormals = gModel["components"][instType]['formals']
            instActuals = instSpec['actuals']
            if instType not in compSpecs:
                raise BuildError('Component type "%s" for instance "%s" is undefined' %
                                      (instType,instName))
            instArgs = self.buildInstArgs(instName,instFormals,instActuals)
            self.components[instName]= Part(self,gModel,instName, instType, instArgs)
...
#        try:
    ...
    def start(self):
        ...
        self.poller.register(self.channel,zmq.POLLIN)
    
        while 1:
            sockets = dict(self.poller.poll())              
            if self.channel in sockets:                     # If there is a message from he discovery service, handle it
                msg = self.channel.recv()
                self.handleServiceUpdate(msg)
                del sockets[self.channel]
            else:
                pass
