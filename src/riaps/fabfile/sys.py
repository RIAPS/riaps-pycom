...
def getDebugInfo():
    ...
    debugFile = '/home/riaps/.riaps/sysdebug-' + hostname + '.log'
    run('echo "#SystemInfo" > ' + debugFile)
    run('echo "Hostname: " >> ' + debugFile)
    run('cat /etc/hostname >> ' + debugFile)
    run('echo "" >> ' + debugFile)
    run('uname -a >> ' + debugFile)
    run('lsb_release -a >> ' + debugFile)
    run('python3 --version >> ' + debugFile)
    run('echo "" >> ' + debugFile)
    run('echo "#Installed Apt Package Info" >> ' + debugFile)
    run('dpkg -l | grep zmq >> ' + debugFile)
    run('dpkg -l | grep riaps >> ' + debugFile)
    run('echo "" >> ' + debugFile)
    run('echo "#RIAPS Configuration (riaps.conf)" >> ' + debugFile)
    sudo('cat /etc/riaps/riaps.conf >> ' + debugFile)
    run('echo "" >> ' + debugFile)
    run('echo "#Installed PIP3 Package Info" >> ' + debugFile)
    run('pip3 list >> ' + debugFile)
    run('echo "" >> ' + debugFile)
    run('echo "#RIAPS Compiled Package Info" >> ' + debugFile)
    run('ls /usr/local/lib/lib* >> ' + debugFile)
    run('echo "" >> ' + debugFile)
    run('echo "#RIAPS Logging Setup Info (riaps-log.conf)" >> ' + debugFile)
    sudo('cat /etc/riaps/riaps-log.conf >> ' + debugFile)
    run('echo "" >> ' + debugFile)
    run('echo "#Redis Configuration (redis.conf)" >> ' + debugFile)
    sudo('redis-server --version >> ' + debugFile)
    sudo('cat /etc/redis/redis.conf >> ' + debugFile)
    run('echo "" >> ' + debugFile)
    get(debugFile,'./')
