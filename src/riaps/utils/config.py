...
class Config(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            logging.warning(
                " Log configuration file %s has a problem: %s."
                % (riaps_logconf, str(e))
            )
            pass
        ...
        try:
            ...
            logger.warning(
                " System configuration file %s has a problem: %s."
                % (riaps_conf, str(e))
            )
            return
        ...
        if files == [] or not c_parse.has_section(riaps_section):
            logger.warning(
                " System configuration file %s not found or invalid file."
                % (riaps_conf)
            )
            return
            ...
            for item in c_parse.items(riaps_section):
                ...
                if hasattr(Config, opt):
                    ...
                    try:
                        ...
                        logger.warning(
                            "Formal and actual type of configuration argument %s differ %s - ignored"
                            % (str(opt), str(optType))
                        )
        except:
