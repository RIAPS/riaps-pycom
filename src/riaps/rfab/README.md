...
- `-v` : Verbose mode
	- Print the stdout streams from all targets as they occur.
- `--role(-r) <rolename>`: Specify target role

TODO: Verify when roles are ignored by hostlist
...
- `riaps_fab` can be called from the `riaps_ctrl` GUI. In this case the `control` host will be where the script is running, and the `nodes` will be determined based on the *currently* connected nodes.
